...
const updateIframe = () => {
  ...
  const rawHtml = sourceRef.value.innerHTML;
  const darkModeClass = isDark.value ? 'dark' : ''
  const bgStyle = isDark.value ? 'background-color: #09090b;' : 'background-color: #ffffff;'

  const fullHtml = `<!DOCTYPE html>
  <html class="${darkModeClass}">
  <head>
    <sty`+`le  type="text/tailwindcss">
...
input:-webkit-autofill:active {
  ...
  <body class="${darkModeClass} p-8 flex flex-wrap items-center justify-center gap-4 m-0">
    ${rawHtml}
  </body>
  </html>`;

  iframeSrc.value = "data:text/html;charset=utf-8," + encodeURIComponent(fullHtml);
}
